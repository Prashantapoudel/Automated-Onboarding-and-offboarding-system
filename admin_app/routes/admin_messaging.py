...
def send_message():
    ...
    if request.method == 'POST':
        ...
        message_data = {
    "sender_id": current_user.id,  # ✅ Use `id` instead of `user_id`
    ...
    "$or": [
        {"sender_id": current_user.id, "recipient_id": selected_recipient},  # ✅ Use `id`
        {"sender_id": selected_recipient, "recipient_id": current_user.id}  # ✅ Use `id`
    ]
}).sort("timestamp", 1))
  # ✅ Sort messages in chronological order
    ...
    return render_template("admin/messages.html", messages=messages, users=all_users, selected_recipient=selected_recipient)
