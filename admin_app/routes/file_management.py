...
from main_app.db_config import onboarding_files_collection, offboarding_files_collection, main_collection, it_collection,offboarding_files_userside_upload_collection, admin_collection
...
@file_bp.route('/view_user_onboarding_files', methods=['GET'])
@login_required
def admin_view_user_onboarding_files():
    ...
    return render_template(
        ...
        get_display_name= get_display_name  # ✅ Pass the function to template
    )
...
def view_offboarding_files():
    """Admins should be able to view ALL offboarding files, from both IT and Users."""
    ...
    if user_id is None:  
        print("🔍 DEBUG: Admin is viewing all offboarding files from all collections.")

        # ✅ Fetch files uploaded by IT/Admin
        it_files = list(offboarding_files_collection.find({}))

        # ✅ Fetch files uploaded by users
        user_files = list(offboarding_files_userside_upload_collection.find({}))

        # ✅ Combine both
        files = it_files + user_files

    else:
        print(f"🔍 DEBUG: User is viewing their own offboarding files. User ID -> {user_id}")

        # ✅ Fetch only user's own files from both collections
        it_files = list(offboarding_files_collection.find({"user_id": user_id}))
        user_files = list(offboarding_files_userside_upload_collection.find({"user_id": user_id}))
        files = it_files + user_files

    # ✅ Add readable uploader and receiver names
    for file in files:
        file["uploaded_by_name"] = get_display_name(file.get("uploaded_by", user_id))
        file["uploaded_for_name"] = get_display_name(file.get("user_id", "Unknown"))

    return render_template("admin/view_offboarding_files.html", files=files, get_display_name= get_display_name, users=all_users)
...
def edit_offboarding_file(file_id):
    """Allow Admin/IT to edit offboarding files from both IT and user uploads."""

    # First, try to find the file in the IT/Admin collection
    file = offboarding_files_collection.find_one({"_id": ObjectId(file_id)})
    source = "admin"

    # If not found, try in the user-side collection
    if not file:
        file = offboarding_files_userside_upload_collection.find_one({"_id": ObjectId(file_id)})
        source = "user"
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Select correct bucket and collection
            if source == "admin":
                bucket = AWS_OFFBOARDING_BUCKET
                collection = offboarding_files_collection
                s3_client = s3_offboarding
            else:
                bucket = AWS_OFFBOARDING_USER_BUCKET
                collection = offboarding_files_userside_upload_collection
                s3_client = s3_offboarding_user  # You should define this in your config

            if new_file:
                # ✅ Upload new file & delete old file
                s3_client.upload_fileobj(new_file, bucket, new_s3_key)
            ...
            collection.update_one(
                ...
                {"$set": {
                    ...
                    "s3_url": f"https://{bucket}.s3.{AWS_S3_REGION}.amazonaws.com/{new_s3_key}",
...
@file_bp.route("/edit-onboarding-file/<file_id>", methods=["GET", "POST"])
@login_required
def edit_onboarding_file(file_id):
    """Allow Admin/IT to edit onboarding files."""

    file = onboarding_files_collection.find_one({"_id": ObjectId(file_id)})

    if not file:
        flash("❌ File not found!", "danger")
        return redirect(url_for("file_bp.view_user_onboarding_files"))
    ...
    if request.method == "POST":
        ...
        if not new_file_name:
            flash("❌ File name cannot be empty!", "danger")
            return redirect(url_for("file_bp.edit_onboarding_file", file_id=file_id))
    ...
    return render_template("admin/edit_onboarding_file.html", file=file)

@file_bp.route("/delete-onboarding-file/<file_id>", methods=["POST"])
@login_required
def delete_onboarding_file(file_id):
    """Allow IT/Admin to delete onboarding files for users."""
    ...
    try:
        ...
        flash("✅ File deleted successfully!", "success")
        return redirect(url_for("file_bp.admin_view_user_onboarding_files"))
...
@file_bp.route('/view_onboarding_files', methods=['GET'])
@login_required
def view_onboarding_files():
    """Admins should be able to view ALL onboarding files, while users see only their own."""
    ...
    # ✅ If Admin, fetch ALL onboarding files
    if user_id is None:  
        print("🔍 DEBUG: Admin is viewing all onboarding files.")
        files = list(onboarding_files_collection.find({}))  # Admin sees all files
    else:
        print(f"🔍 DEBUG: User is viewing their own onboarding files. User ID -> {user_id}")
        files = list(onboarding_files_collection.find({"user_id": user_id}))  # Users see their own files
    ...
    return render_template("admin/view_user_onboarding_files.html", files=files, get_display_name=get_display_name)
